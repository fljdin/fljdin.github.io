...
Au cours de sa vie, tout bon artisan⋅e du Web finit par découvrir les ravages
des attaques par [injection SQL][2], soit par la sagesse de leurs mentors ou soit 
...
![Exploit of a mom](/img/posts/2021-exploits_of_a_mom.png)
...
est connue à l'avance. Toute tentative d'injection est réduite à néant.
...
L'exemple précédent utilise les ordres SQL `PREPARE` et `EXECUTE` mais ce ne
sont pas la seule façon de déclarer une instruction préparée. En réalité, ces
deux clauses s'appuient sur le sous-protocole _Extended Query_ décrit dans la
documentation sur les [flux de messages][3].

Ce protocole encapsule trois messages pour garantir la prise en compte des
paramètres à inclure dans une instruction préparée en avance.
...
haut avec l'exécution d'une requête préparée par `pgbench`. 
...
obtenir le résultat final à retourner au client. Ce fonctionnement permet de 
qualifier la transformation d'une étape à une autre en s'assurant par ailleurs 
que la syntaxe est bonne ou qu'un chemin optimisé puisse être emprunté.
...
L'étape d'analyse de l'instruction lors de l'appel `PREPARE` a un coût : il est
nécessaire de valider la syntaxe, réécrire les jointures si besoin et surtout,
construire le plan d'exécution. Par défaut, une série de plans sera construit
avant que le moteur n'en retienne qu'un seul dont le coût est avantageux. Ce plan 
devient le **plan générique** et sera réutilisé au sein de la même session pour
toutes les exécutions de l'instruction préparée. 

L'utilisation de ce plan d'exécution unique devient la clé pour économiser 
quelques précieuses millisecondes d'analyse à chaque nouvel appel `EXECUTE`.
...
de planification est supérieur au temps d'exécution pour des instructions non 
...
![Répartition des temps d'exécution](/img/posts/2021-repartition-temps-de-reponse.png)
...
De manière générale, il sera toujours bénéfique de préparer ses requêtes, d'autant
plus si votre librairie préférée le supporte. Si vous êtes à la recherche de la
performance, posez-vous les questions suivantes :
